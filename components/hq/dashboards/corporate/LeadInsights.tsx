  ...
  return (
    <Card className="bg-zinc-900/40 border-zinc-700 rounded-none p-8">
      <div className="grid grid-cols-2 md:grid-cols-5 gap-8">
        <MetricItem label="Total Deal" value="45" />
        <MetricItem label="Success" value="12" color="text-emerald-500" />
        <MetricItem label="Discussion" value="28" color="text-amber-500" />
        <MetricItem label="Canceled" value="5" color="text-red-500" />
...
function MetricItem({ label, value, color = "text-white", highlight, text, desc }: any) {
  return (
    <div className="space-y-1">
      <p className="text-[10px] uppercase font-black text-zinc-500 tracking-widest">{label}</p>
      <p className={`text-2xl font-black tracking-tighter ${highlight ? 'text-cyan-400' : color}`}>
